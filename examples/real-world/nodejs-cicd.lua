-- 🚀 Node.js CI/CD Pipeline - Pipeline completo de CI/CD para aplicação Node.js
-- Este exemplo demonstra um pipeline de produção real com múltiplos ambientes
...
describe('API Endpoints', () => {
  ...
  test('GET /health should return health status', async () => {
    ...
    expect(response.body.status).toBe('healthy');
                    ...
                    log.info("✅ Código preparado:")
                    log.info("  📁 Diretório: " .. app_dir)
                    log.info("  📦 package.json criado")
                    log.info("  🐳 Dockerfile criado")
                    log.info("  🧪 Testes criados")
            ...
            
            {
                ...
                description = "Instala dependências e executa build",
                ...
                command = function()
                    ...
                    -- Simular npm install (em produção real executaria npm install)
                    log.info("📥 Simulando npm install...")
                    exec.run("sleep 2") -- Simular tempo de instalação
                    
                    -- Criar node_modules fake para simular instalação
                    local node_modules_dir = app_dir .. "/node_modules"
                    if not fs.exists(node_modules_dir) then
                        fs.mkdir(node_modules_dir)
                    end
                    
                    -- Simular criação de alguns módulos importantes
                    ...
                    -- Simular alguns warnings de lint (realístico)
                    local lint_warnings = {
                        "index.js:15:1 - Warning: Missing semicolon",
                        "index.js:23:5 - Warning: Unused variable 'temp'"
                    }
                    
                    log.info("⚠️  Lint warnings encontrados:")
                    for _, warning in ipairs(lint_warnings) do
                        log.info("  " .. warning)
                    end
                    log.info("✅ Linting concluído (warnings não bloqueiam build)")
                    ...
                    log.info("✅ Build concluído em " .. build_duration .. " segundos")
            ...
            
            {
                ...
                description = "Executa testes unitários e de integração com coverage",
                ...
                command = function()
                    ...
                    log.info("📊 Resultados dos testes unitários:")
                    local unit_passed = 0
                    ...
                    log.info("📊 Cobertura de código:")
                    log.info("  📄 Statements: " .. coverage_data.statements .. "%")
                    log.info("  🌿 Branches: " .. coverage_data.branches .. "%") 
                    log.info("  🔧 Functions: " .. coverage_data.functions .. "%")
                    log.info("  📝 Lines: " .. coverage_data.lines .. "%")
                    
                    -- Verificar se a cobertura atende aos critérios
                    ...
                    if avg_coverage >= min_coverage then
                        log.info("✅ Cobertura de código atende aos critérios (>=" .. min_coverage .. "%)")
                    else
                        log.error("❌ Cobertura insuficiente: " .. string.format("%.1f", avg_coverage) .. "% < " .. min_coverage .. "%")
                    ...
                    log.info("📊 " .. metrics.tests_passed .. "/" .. metrics.tests_run .. " testes executados com sucesso")
            ...
            
            {
                ...
                description = "Constrói e publica imagem Docker",
                ...
                command = function()
                    ...
                    log.info("🏗️  Executando docker build...")
                    ...
                    -- Em produção real, executaria:
                    -- docker.build({tag = image_tag, path = app_dir})
                    -- Por ora, simular o build
                    
                    log.info("✅ Imagem Docker construída")
                    
                    -- Simular scan de segurança da imagem
                    log.info("🔒 Executando scan de segurança da imagem...")
                    exec.run("sleep 2")
                    ...
                    local security_scan = {
                        ...
                        scan_status = "passed" -- passed se não há critical/high críticas
                    }
                    
                    log.info("🛡️  Resultados do scan de segurança:")
                    log.info("  🔴 Critical: " .. security_scan.vulnerabilities.critical)
                    log.info("  🟠 High: " .. security_scan.vulnerabilities.high)
                    log.info("  🟡 Medium: " .. security_scan.vulnerabilities.medium)
                    log.info("  🟢 Low: " .. security_scan.vulnerabilities.low)
                    ...
                    if security_scan.vulnerabilities.high > 2 then
                        log.error("❌ Muitas vulnerabilidades high encontradas!")
                        return false, "Too many high security vulnerabilities"
                    end
                    
                    log.info("✅ Scan de segurança aprovado")
                    
                    -- Simular push da imagem
                    log.info("📤 Fazendo push da imagem para registry...")
                    exec.run("sleep 2")
                    ...
                    log.info("✅ Imagem publicada no registry:")
                    log.info("  📦 " .. image_tag)
                    log.info("  📦 " .. latest_tag)
            ...
            
            {
                ...
                command = function()
                    log.info("🚀 Fazendo deploy para STAGING...")
                    ...
                    log.info("☸️  Aplicando manifesto Kubernetes...")
                    ...
                    -- Simular aplicação do manifesto
                    exec.run("sleep 3")
                    log.info("✅ Manifesto Kubernetes aplicado")
                    
                    -- Simular espera pelo rollout
                    log.info("⏳ Aguardando rollout do deployment...")
                    exec.run("sleep 5")
                    
                    -- Verificar health dos pods
                    log.info("🏥 Verificando saúde dos pods...")
                    ...
                    for _, pod in ipairs(pod_status) do
                        local status_icon = pod.ready and "✅" or "❌"
                    ...
                    if not pods_healthy then
                        log.error("❌ Alguns pods não estão healthy")
                        return false, "Unhealthy pods in staging"
                    end
                    
                    -- Smoke tests no staging
                    log.info("💨 Executando smoke tests no staging...")
                    local http = require("http")
                    ...
                    for _, test in ipairs(smoke_tests) do
                        ...
                        log.info("  ✅ " .. test.name .. " - Status 200 OK")
                    end
                    ...
                    log.info("✅ Deploy para staging concluído!")
                    log.info("  🌍 URL: https://" .. staging_config.domain)
                    log.info("  ⏱️  Tempo de deploy: " .. deploy_duration .. " segundos")
                    log.info("  🔄 Réplicas: " .. staging_config.replicas)
            ...
            
            {
                ...
                command = function()
                    log.info("🧪 Executando testes automatizados no staging...")
                    
                    local config = state.get("pipeline_config")
                    local staging_config = config.environments.staging
                    
                    -- Testes de aceitação automatizados
                    log.info("🤖 Executando testes de aceitação...")
                    ...
                    exec.run("sleep 4") -- Simular execução dos testes
                    ...
                    log.info("📈 Resultados de performance:")
                    log.info("  ⏱️  Tempo médio de resposta: " .. performance_results.response_time_avg)
                    log.info("  📊 P95: " .. performance_results.response_time_p95)
                    log.info("  📊 P99: " .. performance_results.response_time_p99)
                    log.info("  🚀 RPS: " .. performance_results.requests_per_second)
                    log.info("  ❌ Taxa de erro: " .. performance_results.error_rate)
                    log.info("  💾 Uso de memória: " .. performance_results.memory_usage_avg)
                    log.info("  🖥️  Uso de CPU: " .. performance_results.cpu_usage_avg)
                    ...
                    for _, check in ipairs(sla_checks) do
                        if check.value <= check.threshold then
                            log.info("  ✅ " .. check.metric .. ": " .. check.value .. check.unit .. " ≤ " .. check.threshold .. check.unit)
                        else
                            log.info("  ❌ " .. check.metric .. ": " .. check.value .. check.unit .. " > " .. check.threshold .. check.unit)
                    ...
                    if #sla_violations > 0 then
                        log.error("❌ SLA violations encontrados: " .. table.concat(sla_violations, ", "))
                        return false, "SLA violations in staging tests"
                    end
                    
                    -- Gerar relatório de testes de staging
                    ...
                    log.info("✅ Todos os testes de staging passaram!")
                    log.info("📊 " .. acceptance_passed .. "/" .. #acceptance_tests .. " testes de aceitação OK")
                    log.info("⚡ Performance dentro dos SLAs")
            ...
            
            {
                ...
                description = "Gera relatório completo do pipeline",
                depends_on = "run_staging_tests",
                command = function()
                    log.info("📊 Gerando relatório final do pipeline...")
...
🚀 Pipeline CI/CD - Relatório de Execução
========================================
...
⏱️  Execução:
- Duração Total: %d segundos (%.1f minutos)
- Etapas Concluídas: %d/%d (%.1f%%)
...
- Testes Aprovados: %d
- Taxa de Sucesso: %.1f%%
- Cobertura de Código: %.1f%%

🐳 Artifacts:
- Imagem Docker: %s
- Tamanho da Imagem: %s
- Vulnerabilidades: %d críticas, %d high

🚀 Deployments:
- Staging: ✅ %s (%d réplicas)

📊 Métricas de Performance (Staging):
- Response Time P95: %s
- RPS: %d
- Error Rate: %s
- Memory Usage: %s
...
🔄 Pronto para deploy em produção (requer aprovação manual)
]],
                    ...
                    log.info("📈 Pipeline Report Summary:")
                    log.info("  ⏱️  Total Duration: " .. total_duration .. " segundos")
                    log.info("  ✅ Success Rate: " .. string.format("%.1f", success_rate) .. "%")
                    log.info("  🧪 Tests Passed: " .. metrics.tests_passed .. "/" .. metrics.tests_run)
                    log.info("  🚀 Staging Deploy: SUCCESS")
                    log.info("  📄 Relatório: pipeline-report.md")
                    
                    log.info("\n🎉 PIPELINE CI/CD CONCLUÍDO COM SUCESSO!")
                    log.info("✨ Aplicação pronta para deploy em produção")
