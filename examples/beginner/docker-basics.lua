-- 🐳 Docker Basics - Primeiros passos com Docker
-- Este exemplo demonstra operações básicas do Docker

TaskDefinitions = {
    docker_basics = {
        description = "Exemplos básicos de uso do Docker",
        
        tasks = {
            {
                name = "check_docker",
                description = "Verifica se o Docker está funcionando",
                command = function()
                    ...
                    if version_result.success then
                        log.info("✅ Docker instalado! Versão: " .. string.gsub(version_result.stdout, "\n", ""))
                    else
                        log.error("❌ Docker não encontrado: " .. version_result.stderr)
                        return false, "Docker não está disponível"
                    end
                    ...
                    if info_result.success then
                        ...
                        return false, "Docker daemon não acessível"
                    end
                ...
                command = function()
                    ...
                    if running_result.success then
                        if string.len(running_result.stdout) > 0 then
                            ...
                            log.info("📭 Nenhum container rodando no momento")
                        end
                    else
                        log.error("❌ Erro ao listar containers: " .. running_result.stderr)
                    end
            ...
            
            {
                ...
                command = function()
                    log.info("🚀 Executando container hello-world...")
                    ...
                    if run_result.success then
                        log.info("✅ Container executado com sucesso!")
                        log.info("📄 Saída do container:")
                        log.info(run_result.stdout)
                    else
                        log.error("❌ Erro ao executar container: " .. run_result.stderr)
            ...
            
            {
                ...
                command = function()
                    log.info("🧹 Limpando recursos de teste...")
                    ...
                    if rm_result.success then
                        log.info("🗑️  Container removido")
                    else
                        log.info("ℹ️  Container já foi removido automaticamente")
                    end
                    ...
                    if rmi_result.success then
                        log.info("🗑️  Imagem hello-world removida")
                    else
                        log.info("ℹ️  Mantendo imagem hello-world para uso futuro")
                    end
                    --]]
                    
                    log.info("ℹ️  Imagem hello-world mantida para uso futuro")
