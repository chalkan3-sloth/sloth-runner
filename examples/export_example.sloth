...
local main_task = task("export_example_task")
    ...
    :on_success(function(params, output)
        log.info("✅ export_example task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ export_example task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("export_example_secondary")
--     :description("Secondary task for export_example")
--     :depends_on({"export_example_task"})
--     :command(function(params, deps)
...
workflow.define("export_example_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"export_example", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting export_example workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ export_example workflow completed successfully!")
        else
            log.error("❌ export_example workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as export_example.lua.backup
