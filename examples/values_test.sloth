...
local main_task = task("values_test_task")
    ...
    :on_success(function(params, output)
        log.info("✅ values_test task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ values_test task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("values_test_secondary")
--     :description("Secondary task for values_test")
--     :depends_on({"values_test_task"})
--     :command(function(params, deps)
...
workflow.define("values_test_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"values_test", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting values_test workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ values_test workflow completed successfully!")
        else
            log.error("❌ values_test workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as values_test.lua.backup
