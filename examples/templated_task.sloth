...
local main_task = task("templated_task_task")
    ...
    :on_success(function(params, output)
        log.info("✅ templated_task task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ templated_task task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("templated_task_secondary")
--     :description("Secondary task for templated_task")
--     :depends_on({"templated_task_task"})
--     :command(function(params, deps)
...
workflow.define("templated_task_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"templated_task", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting templated_task workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ templated_task workflow completed successfully!")
        else
            log.error("❌ templated_task workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as templated_task.lua.backup
