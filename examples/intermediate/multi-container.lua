-- 🐳 Multi-Container Setup - Trabalhando com múltiplos containers
-- Este exemplo demonstra como orquestrar múltiplos containers Docker

TaskDefinitions = {
    multi_container_setup = {
        description = "Setup e orquestração de múltiplos containers Docker",
        ...
        tasks = {
            {
                ...
                command = function()
                    log.info("🏗️  Preparando ambiente para containers...")
                    ...
                    if network_result.success then
                        log.info("✅ Rede 'sloth-runner-network' criada")
                    else
                        if string.find(network_result.stderr, "already exists") then
                            log.info("ℹ️  Rede 'sloth-runner-network' já existe")
                        else
                            log.error("❌ Erro ao criar rede: " .. network_result.stderr)
                    ...
                    if volume_result.success then
                        log.info("✅ Volume 'sloth-runner-data' criado")
                    else
                        if string.find(volume_result.stderr, "already exists") then
                            log.info("ℹ️  Volume 'sloth-runner-data' já existe")  
                        else
                            log.error("❌ Erro ao criar volume: " .. volume_result.stderr)
            ...
            
            {
                ...
                command = function()
                    ...
                    if redis_result.success then
                        log.info("✅ Container Redis iniciado")
                        
                        -- Aguardar o Redis ficar pronto
                        log.info("⏳ Aguardando Redis ficar pronto...")
                        exec.run("sleep 3")
                        ...
                        if health_check.success and string.find(health_check.stdout, "PONG") then
                            log.info("✅ Redis está respondendo")
                        else
                            log.error("❌ Redis não está respondendo")
                            return false, "Redis não está saudável"
                        end
                        ...
                        log.error("❌ Erro ao iniciar Redis: " .. redis_result.stderr)
            ...
            
            {
                ...
                command = function()
                    log.info("🌐 Iniciando container Nginx...")
                    
                    local docker = require("docker")
                    local config = state.get("container_config")
                    
                    -- Pull da imagem Nginx
                    log.info("⬇️  Baixando imagem Nginx...")
                    local pull_result = docker.exec({"pull", "nginx:alpine"})
                    if not pull_result.success then
                        log.error("❌ Erro ao baixar Nginx: " .. pull_result.stderr)
...
<body>
    <div class="container">
        <h1>🦥 Sloth Runner Multi-Container Demo</h1>
        <div class="status success">
            ✅ Multi-container setup is working!
        </div>
        ...
        <p><em>Generated by Sloth Runner at ]] .. os.date("%Y-%m-%d %H:%M:%S") .. [[</em></p>
                    ...
                    if nginx_result.success then
                        log.info("✅ Container Nginx iniciado")
                        
                        -- Aguardar o Nginx ficar pronto
                        log.info("⏳ Aguardando Nginx ficar pronto...")
                        exec.run("sleep 2")
                        ...
                        if health_result.success then
                            log.info("✅ Nginx está respondendo (Status: " .. health_result.data.status_code .. ")")
                        else
                            log.error("❌ Nginx não está respondendo: " .. health_result.error)
                            return false, "Nginx não está saudável"
                        end
                        ...
                        log.error("❌ Erro ao iniciar Nginx: " .. nginx_result.stderr)
            ...
            
            {
                ...
                description = "Testa comunicação entre containers",
                depends_on = "start_web_container",
                command = function()
                    log.info("🔗 Testando comunicação entre containers...")
                    ...
                    if dns_test.success then
                        log.info("✅ DNS resolution funciona (Nginx -> Redis)")
                    else
                        log.error("❌ Problema de DNS resolution: " .. dns_test.stderr)
                    end
                    ...
                    if connectivity_test.success then
                        log.info("✅ Conectividade de rede funciona (Nginx -> Redis:6379)")
                    else
                        log.info("ℹ️  Teste de conectividade: " .. connectivity_test.stderr)
                    end
                    ...
                    if redis_set.success then
                        ...
                        if redis_get.success then
                            log.info("✅ Dados salvos e recuperados do Redis: " .. redis_get.stdout)
                        end
                    end
                    
                    return true, "Comunicação entre containers testada"
                end
            },
            
            {
                ...
                command = function()
                    log.info("📊 Monitorando containers...")
                    ...
                    if stats_result.success then
                        log.info("📈 Estatísticas de recursos:")
                    ...
                    if network_info.success then
                        log.info("🌐 Rede: " .. network_info.stdout)
                    end
