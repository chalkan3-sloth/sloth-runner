...
local main_task = task("pulumi_example_task")
    ...
    :on_success(function(params, output)
        log.info("✅ pulumi_example task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ pulumi_example task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("pulumi_example_secondary")
--     :description("Secondary task for pulumi_example")
--     :depends_on({"pulumi_example_task"})
--     :command(function(params, deps)
...
workflow.define("pulumi_example_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"pulumi_example", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting pulumi_example workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ pulumi_example workflow completed successfully!")
        else
            log.error("❌ pulumi_example workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as pulumi_example.lua.backup
