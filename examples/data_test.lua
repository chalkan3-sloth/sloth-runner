...
local main_task = task("data_test_task")
    :description("Workflow description - Modern DSL")
    :command(function(params, deps)
        log.info("🚀 Executing data_test with Modern DSL...")
    ...
    :on_success(function(params, output)
        log.info("✅ data_test task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ data_test task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("data_test_secondary")
--     :description("Secondary task for data_test")
--     :depends_on({"data_test_task"})
--     :command(function(params, deps)
...
workflow.define("data_test_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"data_test", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting data_test workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ data_test workflow completed successfully!")
        else
            log.error("❌ data_test workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as data_test.lua.backup
