...
local main_task = task("gcp_pipeline_task")
    ...
    :on_success(function(params, output)
        log.info("✅ gcp_pipeline task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ gcp_pipeline task failed: " .. error)
    end)
    :build()

-- Additional tasks can be added here following the same pattern
-- local secondary_task = task("gcp_pipeline_secondary")
--     :description("Secondary task for gcp_pipeline")
--     :depends_on({"gcp_pipeline_task"})
--     :command(function(params, deps)
...
workflow.define("gcp_pipeline_workflow", {
    ...
    metadata = {
        ...
        category = "general",
        complexity = "basic",
        tags = {"gcp_pipeline", "modern-dsl", "general"},
        created_at = os.date(),
    ...
    on_start = function()
        log.info("🚀 Starting gcp_pipeline workflow...")
        return true
    end,
    
    on_complete = function(success, results)
        if success then
            log.info("✅ gcp_pipeline workflow completed successfully!")
        else
            log.error("❌ gcp_pipeline workflow failed!")
        end
        return true
    end
})
...
-- Original backup saved as gcp_pipeline.lua.backup
