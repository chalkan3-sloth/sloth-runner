...
local main_task = task("{{.WorkflowName}}_main")
    ...
    :command(function(params, deps)
        log.info("🚀 Executing {{.WorkflowName}} workflow...")
    ...
    :on_success(function(params, output)
        log.info("✅ {{.WorkflowName}} task completed successfully")
    end)
    :on_failure(function(params, error)
        log.error("❌ {{.WorkflowName}} task failed: " .. error)
    end)
    :build()
...
workflow.define("{{.WorkflowName}}", {
    ...
    on_complete = function(success, results)
        if success then
            log.info("🎉 {{.WorkflowName}} workflow completed successfully!")
        else
            log.error("❌ {{.WorkflowName}} workflow failed")
        end
        return true
    end
})`
...
local deploy_task = task("deploy")
    ...
    :timeout("10m")
    :on_failure(function(params, error)
        log.error("❌ Deployment failed, consider rollback")
    end)
    :build()
...
local apply_task = task("apply")
    ...
    :command(function(params, deps)
        log.info("🏗️ Applying infrastructure changes...")
...
local build_services = task("build_services")
    ...
    :command(function(params, deps)
        log.info("🔨 Building microservices...")
...
local deploy_production = task("deploy_production")
    ...
    :timeout("15m")
    :on_failure(function(params, error)
        log.error("❌ Production deployment failed, initiating rollback")
    end)
    :build()
...
workflow.define("{{.WorkflowName}}", {
    ...
    on_complete = function(success, results)
        if success then
            log.info("🎉 Microservices deployment completed successfully!")
        else
...
local extract_data = task("extract_data")
    ...
    :command(function(params, deps)
        log.info("📥 Extracting data from sources...")
