...
// NewDocsCommand creates the events docs command
func NewDocsCommand(ctx *commands.AppContext) *cobra.Command {
	var viewMode string

	cmd := &cobra.Command{
		Use:   "docs",
		Short: "Show events command documentation",
		Long:  `Display comprehensive documentation for the events command and all its subcommands.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			viewer := docs.NewDocViewer(docs.ViewMode(viewMode))
			return viewer.ShowCommand("events")
		},
	}
