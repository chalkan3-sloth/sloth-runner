...
// NewStateCommand creates a stub state command for non-CGO builds
func NewStateCommand(ctx *commands.AppContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "state",
		Short: "Manage state and idempotency tracking (requires CGO)",
		Long: `The state command requires CGO support (SQLite) which is not available in this build.

State management features are only available in CGO-enabled builds.
...
This binary can still be used for basic workflow execution and agent management.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return fmt.Errorf("state management requires CGO support (SQLite).\n" +
