...
*   **⚙️ Rich Lua API:** Access system functionalities directly from your Lua tasks:
    ...
    *   **`state` module:** Persistent state management with atomic operations, distributed locks, and TTL support.
    *   **`reliability` module:** Circuit breakers, retry logic with backoff strategies, and failure handling patterns.
...
*   **💾 State Management & Persistence:** SQLite-based persistent state with atomic operations, distributed locks, and TTL support for cross-execution data sharing.
*   **🛡️ Enterprise Reliability Patterns:** Built-in circuit breakers, retry logic with multiple backoff strategies, and failure handling for resilient automation.
*   **📊 Agent Health Monitoring:** Comprehensive system metrics collection including CPU, memory, disk, and custom metrics with configurable thresholds.
